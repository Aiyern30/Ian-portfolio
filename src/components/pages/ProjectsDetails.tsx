...
export default function ProjectsSection() {
  ...
  const getProjectData = (projectId: string) => {
    try {
      return {
        title: t(`projectsData.${projectId}.title`),
        category: t(`projectsData.${projectId}.category`),
        description: t(`projectsData.${projectId}.description`),
      };
    } catch {
  ...
  // Get all categories from translated data
                  ...
                  >
                    ...
                    <div className="absolute top-6 left-6 z-20">
                      <span className="px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-[10px] font-bold text-white tracking-widest uppercase">
                        {isFeatured ? "Featured Project" : projectData.category}
                      </span>
                    </div>
                  </div>
          ...
          >
            ...
            >
              <span className="relative z-10 flex items-center gap-2">
                {showAll
                  ? t("showFewer")
                  : `${t("showAllProjects")} ${filteredProjects.length} ${t("projectsLabel")}`}
                {showAll ? (
            ...
            <p className="text-gray-500 text-sm italic">
              {t("showingOf")} {displayedProjects.length} {t("of")}{" "}
      ...
      <AnimatePresence>
        {isDetailModalOpen && detailProject && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 md:p-10 pointer-events-none">
            {(() => {
              const detailProjectData = getProjectData(detailProject.id);
              return (
                <>
                  <motion.div
                    initial={{ opacity: 0 }}
                  ...
                  <motion.div
                    layoutId={`project-${detailProjectData.title}`}
                    className={cn(
